...
async function getSettings() {
  return (await settingsStore.get("app", { type: "json" })) || {
    companyName: "", certifiedApplicatorName: "", certifiedApplicatorLicense: "", defaultCarrier: "Water"
  };
}
  ...
  try {
    ...
    if (action === "bootstrap") {
      const [settings, clients, chemicals, applications] = await Promise.all([
        getSettings(), listStore(clientsStore), listStore(chemicalsStore), listStore(applicationsStore)
      ]);
      return json(200, { ok: true, data: { settings, clients, chemicals, applications } });
    }
    ...
    if (action === "delete_client") {
      ...
      for (const blob of listed?.blobs || []) {
        ...
        if (app?.clientId === id) await applicationsStore.delete(blob.key);
      }
    ...
    if (action === "replace_all") {
      ...
      const incomingApplications = Array.isArray(payload.entries || payload.applications) ? (payload.entries || payload.applications) : [];
